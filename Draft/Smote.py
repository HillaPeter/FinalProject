...
def Make_Confusion_Matrix(cf,roc=0,
    ...
    # plt.show()
    shaptitle = '/tmp/pycharm_project_723/CM ' + title + '.png'
    print(shaptitle)
    plt.savefig(shaptitle)
...
def SHAP_Iterates_SVMSMOTE( target,title,max_depth=3,n_esti=160,lr=0.1,withexperience = False, color='YlGnBu'):
    ...
    for train_idx, test_idx in gss.split(X, y, groups):
        ...
        shaptitle = '/tmp/pycharm_project_723/SHAP '+title+'.png'
        print(shaptitle)
        plt.savefig(shaptitle)
        # plt.show()
...
matrics_xgb,title = SHAP_Iterates_SVMSMOTE('STSRCOM', 'STSRCOM SVMSMOTE without experience',26,100,0.1,True)
matrics_xgb_df = pd.DataFrame(matrics_xgb)
print(title)
print(matrics_xgb_df)

matrics_xgb,title = SHAP_Iterates_SVMSMOTE('STSRCHOSPD', 'STSRCHOSPD SVMSMOTE without experience',18,500,0.01,True,color='RdPu')
matrics_xgb_df = pd.DataFrame(matrics_xgb)
print(title)
print(matrics_xgb_df)