...
y = df_model_draft['STSRCHOSPD']  # La
print (X.isna().sum())
print(y.isna().sum())
print( range(60, 220, 40))

def hyper_paramytize_optimization(f):
    print ("model with experience with scale pos STSRCHOSPD", file = f)
    print ("--------------------------------------------------------------------", file = f)
    counter = Counter(y)
...
def hyper_paramytize_optimization_under(f):
    print ("model with experience with  under sampling STSRCHOSPD", file = f)
    print ("--------------------------------------------------------------------", file = f)
    counter = Counter(y)
    # estimate scale_pos_weight value
    estimate = counter[0] / counter[1]
...
def hyper_paramytize_optimization_Smote(f):
    print ("model with experience with Smote STSRCHOSPD", file = f)
    print ("--------------------------------------------------------------------", file = f)
    counter = Counter(y)
    # estimate scale_pos_weight value
    estimate = counter[0] / counter[1]
...
def choose_script(model):
    if model == '1':
        name = path + "grid search STSRCHOSPD with experience scale pos.txt"
        print(name)
        f = open(name, 'w')
        hyper_paramytize_optimization(f)
        f.close()
    if model == '2':
        name = path + "grid search STSRCHOSPD with experience under sampling.txt"
        print(name)
        f = open(name, 'w')
        hyper_paramytize_optimization_under(f)
        f.close()
    if model == '3':
        name = path + " grid search STSRCHOSPD with experience SMOTE.txt"
        print(name)
        f = open(name, 'w')
